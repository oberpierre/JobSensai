...
class TestAdapterRegistry(unittest.TestCase):
    ...
    def test_google_extraction_is_learned_not_hand_registered(self):
        # No hand-written Google extraction adapter ships — it is generated by the
        # learning loop, so the registry returns None until that adapter is merged.
        self.assertIsNone(
            self.registry.get_extraction_adapter("https://www.google.com/jobs")
        )
    ...
    def test_has_extraction_adapter(self):
        # No extraction adapter is hand-registered; they are learned, so extraction is
        # reported unsupported until one is generated — which is what triggers learning.
        self.assertFalse(
